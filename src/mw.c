        ...
        if (sensors(SENSOR_BARO))
        {
            if (rcOptions[BOXBARO] && GroundAltInitialized && f.ARMED)
            {
                ...
                if(CopterFlying)                                                // Are we somehow airborne?
                {
                    if(AutostartState)
                    {
                        if (abs_int((int32_t)rcData[THROTTLE] - (int32_t)cfg.rc_mid) > cfg.rc_dbah) // Autostartus interruptus
                        {
                            AutostartState      = 0;
                            ThrFstTimeCenter    = 0;
                            Althightchange      = 0;
...
#define HoverTimeBeforeLand     2000                                            // Wait 2 sec in the air for VirtualThrottle to catch up
        if (sensors(SENSOR_BARO) && f.BARO_MODE && f.ARMED)                     // GroundAltInitialized must not be checked but armed, in case of dumb user -> see above
        {
            ...
            case 4:                                                             // Now climb with desired rate to targethight.
                ...
                AutostartClimbrate = constrain_int((abs_int(AutostartTargetHight - tmp0) / 3), 10, (int16_t)cfg.as_clmbr);// Slow down when getting closer to target
            ...
            case 3:                                                             // Start descent initialize Variables
                ...
                {
                    tmp0 = HoverThrottle - cfg.esc_min;                         // tmp0 contains absolute absolute hoverthrottle
                    if (tmp0 > 0) BaroLandThrlimiter = HoverThrottle + (tmp0 * (int32_t)cfg.al_debounce / 100);// Check here to be on the safer side. Don't set BaroLandThrlimiter if something is wrong
                }
...
// Baro STATS LOGGING
        ...
        {
            float magP;
            if (abs_int((int32_t)rcCommand[YAW]) < 70 && f.MAG_MODE)
            {
        ...
        if (sensors(SENSOR_GPS) && sensors(SENSOR_MAG))                         // Only do GPS stuff if the Mag is available
        {
            ...
            {
                GPS_angle[ROLL]  = constrain_flt((nav[LON] * cos_yaw_x - nav[LAT] * sin_yaw_y) * 0.1f,  -maxbank10, maxbank10);// Nav angles are in DEG * 100
                GPS_angle[PITCH] = constrain_flt((nav[LON] * sin_yaw_y + nav[LAT] * cos_yaw_x) * 0.1f,  -maxbank10, maxbank10);// GPS_angle are expected to be in DEG * 10, I keep that for now
...
*/
        ...
        else
        {
            ...
            {
                errorYW       *= (int32_t)cfg.I8[YAW];
                errorGyroI_YW  = constrain_int(errorGyroI_YW + ((errorYW * cTimeLessJitter) / 3000), -268435454, 268435454); // WindUp & Reference to 3ms looptime
                ITermYW        = constrain_int(errorGyroI_YW >> 13, -250, 250);
            }
        }
        ...
        for (axis = 0; axis < 2; axis++)
        {
            ...
            case 0:
                if (f.ANGLE_MODE || f.HORIZON_MODE)
                {
                    ...
                    PTermACC          = constrain_flt(PTermACC, -tmp0flt, +tmp0flt);
                    errorAngleI[axis] = constrain_flt(errorAngleI[axis] + error * ACCDeltaTimeINS, -30.0f, +30.0f);
...
// But a little modified...
            case 1:                                                             // 1 = New mwii controller (float pimped + pt1element)
                ...
                {
                    tmp0flt = (((int32_t)cfg.rollPitchRate + 27) * (int32_t)rcCommand[axis]) >> 2;
...
static void calculate_Gtune(bool inirun, uint8_t ax)
{
    ...
    else
    {
        ...
        else
        {
            ...
            if (time_skip[ax] > 0)
            {
                if (ax == YAW) AvgGyro[ax] += 32 * ((int16_t)gyroADC[ax] / 32); // Chop some jitter and average
                else AvgGyro[ax] += 128 * ((int16_t)gyroADC[ax] / 128);         // Chop some jitter and average
            }

            if (time_skip[ax] == 16)                                            // ca 48 ms
            {
                AvgGyro[ax] /= time_skip[ax];                                   // AvgGyro[ax] has now very clean gyrodat
                time_skip[ax] = 0;
                ...
                if (cfg.gt_hilimP[ax] && error && OldError[ax] && error != OldError[ax]) // Don't run when not needed or pointless to do so
                {
                    ...
                    {
                        if (abs_int((int32_t)diff_G) > threshP && ax != YAW) result_P64[ax] -= 32; // Don't use antiwobble for YAW
                    }
                    result_P64[ax] = constrain_int(result_P64[ax], (int16_t)cfg.gt_lolimP[ax] << 6, (int16_t)cfg.gt_hilimP[ax] << 6);
...
int32_t abs_int(int32_t x)                                                      // measured 5,9% faster incl. call than inline "abs(x) ((x) > 0 ? (x) : -(x))"
{
    uint32_t mask = x >> 31;
    return (x + mask) ^ mask;
}
...
// http://stackoverflow.com/questions/26692859/best-machine-optimized-polynomial-minimax-approximation-to-arctangent-on-1-1
...
float atan2_fast(float y, float x)
{
    ...
    absX = fabsf(x);
    absY = fabsf(y);
    uint8_t YbiggerX = absY > absX;
    if (YbiggerX) res = absX / absY;
    else res = absY / absX;
    s   = res * res;
    a   = 0x1.7ed22cp-9f;
...
{
    if (abs_int((int32_t)rcvalue) < rcdead) rcvalue = 0;
...
static void DoThrcmmd_DynPid(void)
{
    ...
    {
        prop2 -= constrain_int(((uint16_t)cfg.dynThrPID * (rcData[THROTTLE] - 1500)) / 500, 0, 99);  // Leave at least 1%
    }

    for (i = 0; i < 3; i++)
    {
        tmp   = min(abs_int((int32_t)rcData[i] - (int32_t)cfg.rc_mid), 500);
        prop1 = 100;
        if (i != YAW)                                                       // ROLL & PITCH
        {
            if (cfg.rc_db)
    ...
    tmp = constrain_int(rcData[THROTTLE], cfg.rc_minchk, 2000);
    tmp = (tmp - cfg.rc_minchk) * 1000 / (2000 - cfg.rc_minchk);            // [MINCHCK;2000] -> [0;1000]
    tmp2 = min(tmp / 100, THRLMT);
    rcCommand[THROTTLE] = constrain_int(LKPThr[tmp2] + (tmp - tmp2 * 100) * (LKPThr[tmp2 + 1] - LKPThr[tmp2]) / 100, cfg.esc_min, cfg.esc_max); // [0;1000] -> expo -> [esc_min;esc_max]
}
...
static void DoAirTrim(void)
{
    ...
    case 3:
        ...
        {
            MeasureIni = false;
            if(abs_int((int32_t)Trimtemp[0]) <= Trimlimit && abs_int((int32_t)Trimtemp[1]) <= Trimlimit)// Apply trimming when in range, otherwise keep zero.
            {
...
static void AckTrimCheckTrimLimits(void)
{
    if(abs_int((int32_t)cfg.angleTrim[0]) > Trimlimit || abs_int((int32_t)cfg.angleTrim[1]) > Trimlimit) return; // We may miss Trimlimit a little but no reason to go anal here
    blinkLED(15, 20, 1);
