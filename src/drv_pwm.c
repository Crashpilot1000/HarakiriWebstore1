...
static uint16_t          captures[MAX_RC_CHANNELS];       // slower: static volatile uint16_t captures[MAX_RC_CHANNELS];
...
{
    gpio_config_t cfg;
    if (input) cfg.mode = Mode_IPD;
    else       cfg.mode = Mode_AF_PP;
    cfg.pin   = pin;
...
static void pwmCallback(uint8_t port, uint16_t capture)
{
    ...
    else
    {
        ...
        pwmPorts[port].capture = pwmPorts[port].fall - pwmPorts[port].rise;         // compute capture
        captures[pwmPorts[port].channel] = pwmPorts[port].capture;
        pwmPorts[port].state = 0;                                                   // switch state
        pwmICConfig(timerHardware[port].tim, timerHardware[port].channel, TIM_ICPolarity_Rising);
        failsafeCnt = 0;                                                            // reset failsafe
    }
}
...
void pwmInit(drv_pwm_config_t *init)
{
    ...
    if (init->usePPM) i = 1;                                   // next index is for PPM
    else i = 0;
    ...
    for (i = 0; i < MAX_PORTS; i++)
    {
        ...
        if ((mask & (TYPE_IP | TYPE_IW)) && !init->enablePWMInput) mask = 0;      // hacks to allow current functionality
        if (init->useServos && (port == PWM9 || port == PWM10))    mask = TYPE_S; // remap PWM9+10 as servos
...
{
    if (index < numMotors) *motors[index]->ccr = value;
}

void pwmWriteServo(uint8_t index, uint16_t value)
{
    if (index < numServos) *servos[index]->ccr = value;
}
