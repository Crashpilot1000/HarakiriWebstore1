...
// Mixer
#define MixerShift 14                       // this limits "float" to -1.99f - +1.99f
#define MixerMultiply (1 << MixerShift)
...
typedef enum MultiType
{
    ...
    MULTITYPE_VTAIL4        = 14,
    MULTITYPE_HEX6H         = 15,
    MULTITYPE_CUSTOM        = 16,           // no current GUI displays this
    MULTITYPE_LAST          = 17
} MultiType;
...
/*********** RC alias *****************/
enum                                        // NEVER CHANGE THAT ORDER - WILL BREAK FOR LOOPS AND DO OUT OF BOUNDS MEMORY ERRORS.
{
    ROLL = 0,
    PITCH,
    YAW,
...
typedef struct config_t
{
    ...
    uint8_t	 flt_rp;                        // [0..99] Filtering for Roll and Pitch P&I
    uint8_t	 flt_yw;                        // [0..99] Filtering for Yaw P&I
    ...
    int16_t  esc_min;                       // Set the minimum throttle command sent to the ESC (Electronic Speed Controller). This is the minimum value that allow motors to run at a idle speed.
    int16_t  esc_max;                       // This is the maximum value for the ESCs at full power this value can be increased up to 2000
