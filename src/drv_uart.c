...
void uartInit(uint32_t speed)
{
    ...
    gpio.pin   = Pin_9;                                                           // USART1_TX PA9
    gpio.speed = Speed_2MHz;
    gpio.mode  = Mode_AF_PP;
    gpioInit(GPIOA, &gpio);
    gpio.pin   = Pin_10;                                                          // USART1_RX PA10
    gpio.mode  = Mode_IPU;
    gpioInit(GPIOA, &gpio);
    NVIC_InitStructure.NVIC_IRQChannel                   = DMA1_Channel4_IRQn;    // DMA TX Interrupt
    ...
    DMA_DeInit(DMA1_Channel5);                                                    // Receive DMA into a circular buffer
    ...
    DMA_DeInit(DMA1_Channel4);                                                    // Transmit DMA
    DMA_InitStructure.DMA_PeripheralBaseAddr             = (uint32_t)&USART1->DR;
...
void uart2Init(uint32_t speed, uartReceiveCallbackPtr func, bool rxOnly)
{
    ...
    NVIC_InitStructure.NVIC_IRQChannelCmd                = ENABLE;
    NVIC_Init(&NVIC_InitStructure);
    gpio.pin   = Pin_2;                                                           // USART2_TX PA2
    gpio.speed = Speed_2MHz;
    gpio.mode  = Mode_AF_PP;
    if (!rxOnly) gpioInit(GPIOA, &gpio);
    gpio.pin   = Pin_3;                                                           // USART2_RX PA3
    gpio.mode  = Mode_IPU;
    gpioInit(GPIOA, &gpio);
    uart2Open(speed);
    USART_ITConfig(USART2, USART_IT_RXNE, ENABLE);
    if (!rxOnly) USART_ITConfig(USART2, USART_IT_TXE, ENABLE);
    uart2Callback = func;
}
...
void USART2_IRQHandler(void)
{
    ...
    if (SR & USART_IT_RXNE)
    {
        if (uart2Callback) uart2Callback(USART_ReceiveData(USART2));
    }
