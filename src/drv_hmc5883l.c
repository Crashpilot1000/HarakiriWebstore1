...
void hmc5883lInit(float *calGain)                               // THE RESULT IS ALIGNED
{
    ...
    for (i = 0; i < 10; i++)                                    // Collect 10 samples
    {
        ...
        for (k = 0; k < 3; k++)
        {
            if(abs_int((int32_t)magADC[k]) >= 2047) maggainok = false;// Detect saturation. Official range (-2048, 2047)
...
void hmc5883lRead(int16_t *magData)                             // Read aligned BTW: The 5883 sends in that order: X Z Y
{
    ...
    magData[0] = (int16_t)combine(buf[0], buf[1]);              // X
    magData[2] = (int16_t)combine(buf[2], buf[3]);              // Z
    magData[1] = (int16_t)combine(buf[4], buf[5]);              // Y
