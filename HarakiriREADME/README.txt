...
- Note on "new" and "old" Yaw PIDs (rc_oldyw = 0 or 1): The "D" - part is a limiter to deal with Yaw-jumps. "D" = 0 no limit, increasing Yaw "D" decreases the maximal Yaw "P" - influence (reduces yaw-jumps).
  Yaw "I" is not limited by the "D".
  NEVER use trims on your yaw sticks!! Use the motorstats in CLI to see if your copter has imbalances that need to be tuned out first. Adjust mixing if non symetrical copter is at play.
...
- Reworked frame constants/factor mixing. Constants were "float" before now they are 16 Bit to save much eeprom space. 16Bit is just used for storage of the factors.
  The custom mixer "cmix" is still displayed in "float" but can not exceed the range limit of -1.9999f -> +1.9999f. Keep that in mind when setting up a custom mixer.
- Removed all Airplane stuff. So it is copter only.
